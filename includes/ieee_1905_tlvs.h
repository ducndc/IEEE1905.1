...
#ifndef _IEEE_1905_TLVS_H_
#define _IEEE_1905_TLVS_H_
...
#define MUL_CAST_1905		((uint8_t *)"\x01\x80\xC2\x00\x00\x13")
#define MUL_CAST_LLDP		((uint8_t *)"\x01\x80\xC2\x00\x00\x0E")
...
#define LINK_METRIC_QUERY_NEIGHBOR_ALL				(0x00)
#define LINK_METRIC_QUERY_NEIGHBOR_SPECIFIC			(0x01)

#define LINK_METRIC_QUERY_TYPE_TX					(0x00)
#define LINK_METRIC_QUERY_TYPE_RX					(0x01)
#define LINK_METRIC_QUERY_TYPE_BOTH					(0x02)
...
struct tlv_non1905_neighbor {
	uint8_t local_mac_addr[MAC_LEN];
...
struct tx_link_info {
	uint8_t local_mac_addr[MAC_LEN];
	uint8_t neighbor_mac_addr[MAC_LEN];
	uint16_t media_type;		/* one of MEDIA_TYPE_* */
	uint8_t has_bridge;
	uint32_t errors;
	uint32_t packets;
...
struct tlv_tx_linkmetric {
	uint8_t al_addr[MAC_LEN];
	uint8_t neighbor_al_addr[MAC_LEN];
...
struct tlv_device_identification {
	...
	uint8_t manufacturer[NAME_LEN];
	uint8_t model[NAME_LEN];
} __attribute__ ((packed));
...
struct ipv4_entry {
	uint8_t type;
	uint8_t address[IPV4_ADDR_LEN];
	uint8_t dhcpserver[IPV4_ADDR_LEN];
} __attribute__ ((packed));
...
struct ipv6_entry {
	uint8_t type;
	uint8_t address[IPV6_ADDR_LEN];
	uint8_t origin[IPV6_ADDR_LEN];
} __attribute__ ((packed));

struct ipv6_interface {
	uint8_t mac_addr[MAC_LEN];
	uint8_t link_local_address[IPV6_ADDR_LEN];
	uint8_t num_ipv6;
