	...
	void **key)
{
	...
	buf = static_cast<uint8_t *>(calloc(1000, sizeof(uint8_t)));

	if (!buf) {
		return -ENOMEM;
	}

	wkey = static_cast<wsc_key *>(calloc(1, sizeof(*wkey)));
	...
	/* generate keys */
	if (PLATFORM_GENERATE_DH_KEY_PAIR(&wkey->key, &wkey->keylen, &wkey->pub, &wkey->publen) != 0) {
		free(wkey);
		free(buf);
		return -1;
	}

	memcpy(wkey->macaddr, in->macaddr, 6);
	get_random_bytes(16, nonce_e);
	memcpy(wkey->nonce, nonce_e, 16);
	...
	if (wsc_put_u8(&p, &rem, ATTR_VERSION, 0x10) ||
	    ...
	    wsc_put(&p, &rem, ATTR_PUBLIC_KEY, wkey->pub, wkey->publen) ||
	...
	uint16_t *m2_size)
{
	...
	buffer = static_cast<uint8_t *>(calloc(1000, sizeof(uint8_t)));
	...
	/* encrypted settings */
	{
		...
		for (i = 0; i < num_ven_ies; i++) {
			if (ven_ies && num_ven_ies > 0 && ven_ies[i].len < rlen) {
				...
				ie_buf = static_cast<uint8_t *>(calloc(1, ie_buflen));
			...
			case ATTR_VENDOR_EXTENSION:
				{
					...
					if (id[0] == WFA_VENDOR_ID_1 && id[1] == WFA_VENDOR_ID_2 && id[2] == WFA_VENDOR_ID_3) {
						...
						if (!*ext) {
							*ext = static_cast<uint8_t *>(calloc(1, attr_len + 4));
							...
							tmp = static_cast<uint8_t *>(realloc(*ext, *extlen + attr_len + 4));
