...
#define T2STR(t)	case TLV_TYPE_ ## t: return #t;
	...
	T2STR(PUSH_BUTTON_EVENT_NOTIFICATION)
	T2STR(PUSH_BUTTON_JOIN_NOTIFICATION)
	T2STR(GENERIC_PHY_DEVICE_INFORMATION)
	T2STR(DEVICE_IDENTIFICATION)
	T2STR(CONTROL_URL)
	T2STR(IPV4)
	T2STR(IPV6)
	T2STR(GENERIC_PHY_EVENT_NOTIFICATION)
	T2STR(1905_PROFILE_VERSION)
	T2STR(POWER_OFF_INTERFACE)
	T2STR(INTERFACE_POWER_CHANGE_INFORMATION)
	T2STR(INTERFACE_POWER_CHANGE_STATUS)
	T2STR(L2_NEIGHBOR_DEVICE)
	}
...
tlv_alloc(
	uint16_t datalen)
{
	struct tlv *n = static_cast<struct tlv *>(calloc(1, sizeof(*n) + datalen));
...
{
    static const size_t sizeof_tlv[TLV_TYPE_L2_NEIGHBOR_DEVICE + 1] = {0};

    // Only initialize once
    if (sizeof_tlv[TLV_TYPE_END_OF_MESSAGE] == 0) {
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_END_OF_MESSAGE]) = sizeof(struct tlv_eom);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_AL_MAC_ADDRESS_TYPE]) = sizeof(struct tlv_al_addr);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_MAC_ADDRESS_TYPE]) = sizeof(struct tlv_mac_addr);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_DEVICE_INFORMATION_TYPE]) = sizeof(struct tlv_device_info);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_DEVICE_BRIDGING_CAPABILITIES]) = sizeof(struct tlv_device_bridge_caps);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_NON_1905_NEIGHBOR_DEVICE_LIST]) = sizeof(struct tlv_non1905_neighbor);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_NEIGHBOR_DEVICE_LIST]) = sizeof(struct tlv_1905neighbor);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_LINK_METRIC_QUERY]) = sizeof(struct tlv_link_metric_query);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_TRANSMITTER_LINK_METRIC]) = sizeof(struct tlv_tx_linkmetric);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_RECEIVER_LINK_METRIC]) = sizeof(struct tlv_rx_linkmetric);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_VENDOR_SPECIFIC]) = sizeof(struct tlv_vendor_specific);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_LINK_METRIC_RESULT_CODE]) = sizeof(struct tlv_link_metric_result);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_SEARCHED_ROLE]) = sizeof(struct tlv_searched_role);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_AUTOCONFIG_FREQ_BAND]) = sizeof(struct tlv_autoconfig_band);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_SUPPORTED_ROLE]) = sizeof(struct tlv_supported_role);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_SUPPORTED_FREQ_BAND]) = sizeof(struct tlv_supported_band);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_WSC]) = sizeof(struct tlv_wsc);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_PUSH_BUTTON_EVENT_NOTIFICATION]) = sizeof(struct tlv_pbc_notification);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_PUSH_BUTTON_JOIN_NOTIFICATION]) = sizeof(struct tlv_pbc_join_notification);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_GENERIC_PHY_DEVICE_INFORMATION]) = sizeof(struct tlv_generic_phy_devinfo);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_DEVICE_IDENTIFICATION]) = sizeof(struct tlv_device_identification);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_CONTROL_URL]) = sizeof(struct tlv_control_url);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_IPV4]) = sizeof(struct tlv_ipv4);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_IPV6]) = sizeof(struct tlv_ipv6);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_GENERIC_PHY_EVENT_NOTIFICATION]) = sizeof(struct tlv_pbc_generic_phy_notification);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_1905_PROFILE_VERSION]) = sizeof(struct tlv_1905_profile);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_POWER_OFF_INTERFACE]) = sizeof(struct tlv_power_off);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_INTERFACE_POWER_CHANGE_INFORMATION]) = sizeof(struct tlv_power_change_request);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_INTERFACE_POWER_CHANGE_STATUS]) = sizeof(struct tlv_power_change_status);
        const_cast<size_t&>(sizeof_tlv[TLV_TYPE_L2_NEIGHBOR_DEVICE]) = sizeof(struct tlv_l2_neighbor);
    }
...
#define CMDU_RESERVE_HEADSPACE	32
	...
	p = static_cast<uint8_t *>(calloc(1, sizeof(*n) + size + CMDU_RESERVE_HEADSPACE));
	...
	size_t size)
{
	...
	n = static_cast<uint8_t *>(realloc((uint8_t *)c, sizeof(*f) + size + CMDU_RESERVE_HEADSPACE));
	...
	int policy_len)
{
	...
	cmdu_for_each_tlv(t, c->data, len) {
		for (i = 0; i < policy_len; i++) {
			...
			if (policy[i].minlen > 0 && (tlv_length(t) < policy[i].minlen)) {
				ieee1905_set_error(CMDU_STATUS_ERR_TLV_LEN_INSUFFICIENT);
				return -1;
			}

			if (policy[i].maxlen > 0 && (tlv_length(t) > policy[i].maxlen)) {
			...
			if (tv[i][0]) {
				if (policy[i].present == TLV_PRESENT_ONE || policy[i].present == TLV_PRESENT_OPTIONAL_ONE) {
	...
	for (i = 0; i < policy_len; i++) {
		if ((policy[i].present == TLV_PRESENT_ONE || policy[i].present == TLV_PRESENT_MORE) && !tv[i][0]) {
